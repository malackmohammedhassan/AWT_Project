...
function MiniHistogram({ values }: { values: { x: number; y: number }[] }) {
  ...
  return (
    <svg viewBox="0 0 120 70" className="h-full w-full">
      {values.map((item, idx) => {
        ...
        return <rect key={item.x} x={x} y={66 - height} width={width} height={height} fill="#10b981" opacity="0.75" />;
      })}
    </svg>
  );
}

export function HistogramMatrix() {
  const { selectedDatasetId } = useSelectedDataset();
  const { data, isLoading, isError, error, refetch } = useDatasetStats(selectedDatasetId);

  const cells = useMemo(() => data?.histogramAll ?? [], [data]);
  ...
  return (
    <section className="rounded-xl border border-slate-700 bg-slate-800 p-4 shadow-md">
      <h3 className="mb-3 text-xl font-medium text-slate-100">Histogram Matrix</h3>
      <div className="grid grid-cols-3 gap-2">
        {cells.map((cell) => (
          <article key={cell.label} className="rounded-md border border-slate-700 bg-slate-900/60 p-2">
            ...
            <div className="h-16">
              <MiniHistogram values={cell.bins} />
            </div>
          </article>
        ))}
      </div>
